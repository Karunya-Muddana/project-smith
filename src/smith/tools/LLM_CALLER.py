...
try:
    ...
    if API_KEY:
        ...
        # Client abstraction is different in this lib version, usually we get model directly
        # But to keep existing structure, we'll set a flag.
        client = True 
    else:
        init_error = "Missing GOOGLE_API_KEY environment variable."
except ImportError:
    init_error = "Module 'google.generativeai' not found. Install with `pip install google-generativeai`."
    ...
    try:
        ...
        if hasattr(response, "candidates") and response.candidates:
            ...
            if (
                ...
                and str(candidate.finish_reason) != "STOP"
            ):
                ...
                if current_model in VALID_MODELS:
                    try:
                        ...
                        if current_idx + 1 < len(VALID_MODELS):
                            ...
                            logger.warning(
                                f"Model '{current_model}' blocked. Switching to: {fallback}"
                            )
...
def call_llm(prompt: str, model: str = None):
    ...
    try:
        ...
        if text_output.startswith("[BLOCKED]"):
