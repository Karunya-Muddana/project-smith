...
def normalize_articles(
    articles: Union[List[Dict[str, str]], List[str]],
) -> List[Dict[str, str]]:
    """
    ...
    for article in articles:
        if isinstance(article, dict):
            ...
            normalized.append(
                {"title": article, "content": "", "snippet": "", "url": article}
            )
        else:
...
def cluster_articles(
    articles: List[Dict[str, str]], similarity_threshold: float = 0.3
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for article in articles:
            ...
            if keywords:  # Only include if we found keywords
                article_keywords.append({"article": article, "keywords": keywords})

        if not article_keywords:
            return {
                "status": "error",
                "error": "Could not extract keywords from any articles",
            }
        ...
        return {
            ...
            "metrics": {
                "largest_cluster": (
                    result_clusters[0]["count"] if result_clusters else 0
                ),
                "smallest_cluster": (
                    result_clusters[-1]["count"] if result_clusters else 0
                ),
                "avg_cluster_size": (
                    round(len(articles) / len(result_clusters), 1)
                    if result_clusters
                    else 0
                ),
            },
...
def run_clustering_tool(
    articles: List[Union[Dict[str, str], str]], similarity_threshold: float = 0.3
):
    """
