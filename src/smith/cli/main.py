    ...
    with Progress(
        ...
        for event in smith_orchestrator(user_input, require_approval=False):
            ...
            if event_type == "status":
                ...
                progress.update(main_task, description=f"[green]✓ Plan created ({total_steps} steps)", total=total_steps, completed=0)
                
            elif event_type == "step_start":
                step_idx = event.get("step_index", 0)
                tool = event.get("tool", "unknown")
                progress.update(main_task, description=f"[bold]→ Step {step_idx + 1}/{total_steps}:[/bold] [cyan]{tool}[/cyan]", completed=step_idx)
                ...
                progress.update(main_task, description="[green]✓ Complete", completed=total_steps)
                    
            elif event_type == "error":
                error_msg = event.get("message", "Unknown error")
                progress.update(main_task, description=f"[red]✗ Error:[/red] {error_msg}")
